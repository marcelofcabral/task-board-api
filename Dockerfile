...
FROM dhi.io/python:3.14-debian-dev AS build-stage

# copy uv binaries from the uv image into this stage's image
COPY --from=dhi.io/uv:0-debian-dev /usr/local/bin/uv /usr/local/bin/uvx /usr/local/bin/
...
FROM dhi.io/python:3.14-debian AS runtime-stage
